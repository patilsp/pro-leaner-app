<?php
    ...
    $userId = $_SESSION['cms_userid'];
    $token = $_SESSION['token'];
    $links = array();
    $sql1 = "SELECT * FROM users_module_access WHERE user_id = ?";
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav">
        ...
        while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $query1 = "SELECT * FROM permission WHERE (level = 2 AND parent = '".$row["id"]."') AND (parent != 48) AND visible = 1 ORDER BY sequence";
