...
// include "configration/config_b2c.php";
            ...
            <table id="datatable" class="table table-striped table-bordered sourced-data">
              ...
              <tbody>
                <?php
                  ...
                  /*echo "<pre/";
                  print_r($content4student_topic_ids);*/
                  ...
                  //Get B2C Topic Names
                  $oldTopics = array();
                  $query = "SELECT id, fullname FROM sp_2021_master.mdl_course";
                  $stmt = $db->query($query);
                  while($fetch = $stmt->fetch(PDO::FETCH_ASSOC)){
                    $oldTopics[$fetch['id']] = $fetch['fullname'];
                  }

                  $i = 1;
                  foreach ($getTaskList as $task) {
                    if($task['status'] == "Publish" && in_array($task['task_id'], $published_task_ids)){

                      if(in_array($task['topic_id'], $content4student_topic_ids))
                ...
                ?>
                  <tr style="<?php echo $tr_bg_color; ?>">
                    ...
                    <td><?php echo $oldTopics[$task['topic_id']]; ?></td>
                    ...
                    <td>
                      <?php
                        ...
                        elseif ($task['status'] == "Publish") 
                          $status_btn_lable = "Production Ready";
                        elseif ($task['status'] == "Released") 
                          $status_btn_lable = "Released";
                        else
