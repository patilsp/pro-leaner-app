<?php
  ...
  if(isset($_FILES['img']))
  {
    if($_FILES['img']['error'][0] != 4)
    {
      foreach($_FILES['img']['name'] as $k=>$value)
      {
        ...
        {
          array_push($only_image_paths, $filepath1);
  ...
  $text1 = "";
  //print_r($lp1);
  foreach($lp1 as $key => $value){
    $li = $value;
    if($li != ""){
        $text1.='<li> <img id="_objective1" src="images/bullet_point/bullet2.png"  style="float:left">
  ...
  $text2 = "";
  //print_r($lp2);
  foreach($lp2 as $key => $value){
    $li = $value;
    if($li != ""){
        $text2  .='<li> <img id="_objective2" src="images/bullet_point/bullet2.png"  style="float:left">
          <p style="margin-left:30px;">'.$li.'</p>
        </li>';
      } else {
        continue;
  ...
  foreach($lp3 as $key => $value){
    $li = $value;
    if($li != ""){
        $text3  .='<li> <img id="_objective3" src="images/bullet_point/bullet2.png"  style="float:left">
          <p style="margin-left:30px;">'.$li.'</p>
        </li>';
      } else {
        continue;
...
<link href="css/lessons/011lscl02.css" rel="stylesheet" type="text/css" />
</head>
      ...
      <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12 template img-responsive">
          ...
          <div class="col-md-12 col-sm-12 col-xs-12 content_part">
            <div class="col-md-6 col-sm-6 col-xs-12 list_1">
              <ul class="_content2">
                 '.$text1.'
              </ul>
            </div>
            <div class="col-md-6 col-sm-6 col-xs-12 _content2"> <img class="img-responsive img1" src="images/'.$_FILES['img']['name'][0].'"  /> </div>
          </div>
          </div>
          
          <div class="col-md-12 col-sm-12 col-xs-12 content_part">
            <div class="col-md-6 col-sm-6 col-xs-12 _content3"> <img class="img-responsive img2" src="images/'.$_FILES['img']['name'][1].'"  /> </div>
            <div class="col-md-6 col-sm-6 col-xs-12 list_2">
              <ul class="_content3">
                 '.$text2.'
              </ul>
            </div>
          ...
          <div class="col-md-12 col-sm-12 col-xs-12 content_part">
            <div class="col-md-6 col-sm-6 col-xs-12 list_3">
              <ul class="_content4">
                 '.$text3.'
              </ul>
            </div>
            <div class="col-md-6 col-sm-6 col-xs-12 _content4"> <img class="img-responsive img3" src="images/'.$_FILES['img']['name'][2].'"  /> </div>
          </div>
        </div>
      </div>
    </div>
...
if($action_type == "preview") {
  ...
  $file_path_html_css = array();
  $file_path_html_css[] = $dir_root.'app/'.$saving_path.'/'.$css_push_pathh.'/'.$newFileName.'.css';
  $file_path_html_css[] = $dir_root.'app/'.$saving_path.'/'.$newFileName.'.html';
  foreach($only_image_paths as $path) {
    $file_path_html_css[] = $path;
  }
  $external_files = json_encode($file_path_html_css);
  
  $status =true;
  $external_files =$external_files;
  $response = array("status"=>$status, "external_files"=>$external_files);
