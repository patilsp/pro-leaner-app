		...
		if (result.length > 1) {
			setTimeout(function(){
				...
				for(var i = 0; i< result.length; i++) {
					if(result[i] == "item"+correctAnswers[i]) {
	...
	setTimeout(function () {
	    ...
	    $.ajax({
	        ...
	        success:function(data)
	        {
	          ...
	          $(".exp_content").text(data.explanation);
            ...
            $.each(data, function(i, e){
        		...
        		//console.log("original_opt_array - ", original_opt_array);
      			...
      			$.each(original_opt_array, function(key, val){
      				...
      				correctAnswers[key] = index + 1;
      			});
      			//console.log(correctAnswers);

        		$.each(shuffle_options, function(key, val){
    					$("#options tbody").append(`<tr class="d-flex align-items-center" id="item`+(key+1)+`">
                  <td class="col-12" id="last_td`+(key+1)+`">
                    <ul>
