$(function () {
    ...
    setTimeout(function () {
        ...
        $.ajax({
            ...
            success:function(data)
            {
              ...
              $.each(data, function(i, e){
                  ...
                  if(i == 'qust1') {
                    $.each(e, function(key, val){
                    	if(key == "qust1"){
                    		var textReplacetoIP = val.replace('[blank]', '<span><input type="text" class="form-control" id="qust1_ip" value=""></span>');
                    		$("#qus1_loop").append(textReplacetoIP.replace('<p>','<p class="input-group-text">'));
                    ...
                    $.each(e, function(key, val){
                    	if(key == "qust2"){
                    		var textReplacetoIP = val.replace('[blank]', '<span><input type="text" class="form-control" id="qust2_ip" value=""></span>');
                    		$("#qus2_loop").append(textReplacetoIP.replace('<p>','<p class="input-group-text">'));
                    	} else if(key == "scramble_word"){
                    		scramble_word_array.push(val);
                    	} else if(key == "unscramble_word"){
                    		unscramble_word_array.push(val);
                    ...
                    $.each(e, function(key, val){
                    	if(key == "qust3"){
                    		var textReplacetoIP = val.replace('[blank]', '<span><input type="text" class="form-control" id="qust3_ip" value=""></span>');
                    		$("#qus3_loop").append(textReplacetoIP.replace('<p>','<p class="input-group-text">'));
                    	} else if(key == "scramble_word"){
                    		scramble_word_array.push(val);
                    	} else if(key == "unscramble_word"){
                    		unscramble_word_array.push(val);
              ...
              setTimeout(function(){
              	unscram_word(displayQuestion, scramble_word_array[0].toLowerCase());
              }, 500)                  
          },
            beforeSend: function(){
    ...
    function unscram_word(qust_num, $scramble_word) {
		var $scramble_word = $scramble_word.toLowerCase();
		if(qust_num === 1){
