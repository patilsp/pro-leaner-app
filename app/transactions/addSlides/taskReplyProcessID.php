	...
	try{
		...
		if($up_status_cw == 5){
          	...
          	if( !in_array($topic_id, $content4student_topic_ids) ){
				...
				foreach($records as $k1=>$record) {
					...
					while($rowsSch = $stmtSch->fetch(PDO::FETCH_ASSOC))
					{
						...
						if($k1 == 0) {
							DeleteRecord("$thisdb.lesson_pages_audio", array("course_id"=>$topic_id));
						}
					}
			...
			foreach($records as $record)
			{
				...
				$json = json_decode($record['slide_json']);

				if($json->audio) {
					$audioPath = str_replace($web_root."app/","",$json->audio);
				} else {
					$audioPath = "";
				}
				...
				$inserted_pageid = insertUpdateSlide($lesson_id, $prev_slide_id, $html_file1, $title, $audioPath, $topic_id);
			...
			CopyAudiosToSkills4life($html_file, $topic_name, $class_id);
