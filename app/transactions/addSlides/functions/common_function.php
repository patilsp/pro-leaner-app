<?php
				    ...
				    while($fetch = $stmt->fetch(PDO::FETCH_ASSOC)){
			          ...
			          if(count($slides) > 0) {
			            foreach ($slides as $slide) {
			              ...
			              $query_ln = "SELECT name  FROM resources WHERE id='".$layout_id."' AND resource_type_id=5";
				              ...
				              <div class="col-md-12">
				                <div class="card bd-0 shadow-base">
				                  ...
				                  <div class="pd-y-40 pd-x-30 tx-center">
				                  	<button type="button" data-taskid="'.$taskid.'" data-autoid="'.$id.'" data-topic_id="'.$topic_id.'" data-slidepath="'.$slide_file_path.'" data-classid="'.$classid.'" data-layoutid="'.$layout_id.'" data-layoutname="'.$layout_name.'" class="btn btn-dafault btn-oblong btn-block bd-0 tx-11 tx-semibold pd-y-12 tx-mont tx-uppercase tx-spacing-1 clickSavedSlides" style="white-space: normal;">'.$slideContentContains.$layout_id_name.$slide_title.'</button>
				                  </div>
				                </div>
				              </div>
				            </div>
		...
		try{
			...
			while($fetch1 = $stmt1->fetch(PDO::FETCH_ASSOC))
			{
				...
				//print_r($slide_ids);
		  		...
		  		foreach($slide_ids as $key=>$slide_id)
		  		{
		  			...
		  			if(in_array($next_slide, $new_slides_prev_page_ids)) {
		  				...
		  				$add_slide_list = GetRecords("add_slide_list", array("prev_slide_id"=>$next_slide, "lesson_id"=>$fetch1['id']), array("sequence"));
		  				if(count($add_slide_list) > 0) {
		  					foreach($add_slide_list as $slide)
		  					{
			  					if($slide['status'] == 13)
			  						$new_slide = false;
			  					else {
				...
				if(in_array($next_slide, $new_slides_prev_page_ids)) {
	  				//next_slide exists in add_slide_list table means fetch the path from the table
	  				$prev_slide_id_array = array();
	  				$add_slide_list = GetRecord("add_slide_list", array("prev_slide_id"=>$next_slide, "lesson_id"=>$fetch1['id']));
	  				if(isset($add_slide_list['id'])) {
	  					if($add_slide_list['status'] == 13)
	  						$new_slide = false;
	  					else {
	  						$new_slide = true;
	...
	function updateSlideJson($id, $data, $slide_html_path) {
		global $db;
		try{
			$query = "UPDATE add_slide_list SET slide_json = ?, slide_file_path = ? WHERE id = ?";
