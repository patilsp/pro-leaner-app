<?php
  ...
  try {
    ...
    $getSlideJson = GetRecord("cpadd_slide_list", array("id"=>$slideid));
    ...
    $slideLayoutHTML = $web_root.str_replace('contents', 'cpcontents', $getSlideLayoutHTML['layoutfilepath_html']).'?api_end_point=app/transactions/quizzone/getactivitydata.php&qust_id='.$slideid;
    $slideLayoutPHP = $web_root.str_replace('contents', 'cpcontents', $getSlideLayoutHTML['layoutfilepath_html']);
    ...
    if(array_key_exists("audio", $objectToArray)) {
      ...
      $getresources = GetRecord("resources", array("cp_slide_id"=>$slideid));
      $resId = $getresources["id"];
    }
