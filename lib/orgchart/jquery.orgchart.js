(function($) {
    ...
    function OrgChart($container, opts){
        ...
        this.draw = function(){
            ...
            if(opts.allowEdit){
                $container.find('.node h2').click(function(e){
                    ...
                    //the below if condition added by dinesh for disable to edit parent node(ex - Class Name)
                    if(parseInt(nodes[thisId].data.parent) !== 0){
            ...
            $container.find('.org-add-button').click(function(e){
                ...
                if(self.opts.onAddNode !== null){
                    //console.log('onAddNode', nodes[thisId]);
        ...
        this.startEdit = function(id){
            var prevVal = "";
            var inputElement = $('<input class="org-input" onkeypress="return Validate(event);" type="text" value="'+nodes[id].data.name+'"/>');
            ...
            inputElement.focus(function (event) {
                if(inputElement.val() == 'Enter subject' || inputElement.val() == 'Enter chapter' || inputElement.val() == 'Enter topic' || inputElement.val() == 'Enter subTopic'){
                    prevVal = inputElement.val();
                    inputElement.val("");
                }
            });
            ...
            inputElement.blur(function(event){
                ...
                if(nodes[id].data.name == ""){
                    nodes[id].data.name = prevVal;
                }
                self.saveNode(id, nodes[id].data.name,'name');
            })
        }

        this.startSeqEdit = function(id){
            var prevVal = "";
            var inputElement = $('<input class="org-input"  type="text" value="'+nodes[id].data.sequence+'"/>');
            $container.find('div[node-id='+id+'] h3').replaceWith(inputElement);
            var commitChange = function(){
                var h2Element = $('<h3>'+nodes[id].data.sequence+'</h3>');
                if(opts.allowEdit){
                    h2Element.click(function(){
                        self.startSeqEdit(id);
                    })
                }
                inputElement.replaceWith(h2Element);
            }  
            ...
            inputElement.keyup(function(event){
                if(event.which == 13){
                    ...
                    nodes[id].data.sequence = inputElement.val();
                }
            });
            inputElement.blur(function(event){
                commitChange();
                //console.log(nodes[id].data.sequence);
                if(nodes[id].data.sequence == ""){
                    nodes[id].data.sequence = prevVal;
                }
                self.saveNode(id, nodes[id].data.sequence, 'seq');
            })

           
        }
       
        //custom code added by dinesh for to save.
        this.saveNode = function(id, name, category){
            // console.log(id,'----',name);
            saveCategory(id, 'update', name, category);
        }
    ...
    function Node(data){
        ...
        this.formatNode = function(opts){
            ...
            if(typeof data.sequence !== 'undefined'){
                nameString = nameString+'<h3>'+self.data.sequence+'</h3>';
            }
            if(typeof data.description !== 'undefined'){
                descString = '<p>'+self.data.description+'</p>';
            }
            if(opts.showControls && typeof self.data.type !== 'undefined'){
                var label = '';
                if(self.data.type == 'class')
                    label = 'Class';
                else if(self.data.type == 'subject')
                    label = 'Subject';
                else if(self.data.type == 'chapter')
                    label = 'Chapter';
                else if(self.data.type == 'topic')
                    label = 'Topic';

                var buttonsHtml = "<div class='org-add-button'>"+opts.newNodeText+"</div><div class='label'>"+label+"</div>";
                //the below if condition added by dinesh for show and hide this buttons for parent node as well as last node.
                if(parseInt(self.data.parent) !== 0 && self.data.type !== 'subTopic') {
                    buttonsHtml = "<div class='org-add-button'>"+opts.newNodeText+"</div><div class='label'>"+label+"</div><div class='org-del-button'></div>";
                    //console.log('self.data.type----', self.data.type);
                } else if(self.data.type === 'subTopic') {
                    buttonsHtml = "<div class='label'>Sub Topic</div><div class='org-del-button'></div>";
                }
                
            } else if (typeof self.data.type === 'undefined') {
                buttonsHtml = "<div class='org-del-button'></div>";
            } else{
                buttonsHtml = '';
            }
            return "<div class='node' node-id='"+this.data.id+"' type='"+this.data.type+"'>"+nameString+descString+buttonsHtml+"</div>";
        }
    }
